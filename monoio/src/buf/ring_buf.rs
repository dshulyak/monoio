...
/// Builder for [`MmapedRingBuf`].
pub struct MmapedRingBufBuilder {
    buffer_count: u32,
    buffer_size: usize,
    group_id: u16,
    silent_drop: bool,
...
impl Default for MmapedRingBufBuilder {
    fn default() -> Self {
        Self::new()
    }
}

impl MmapedRingBufBuilder {
    ...
    /// Silently unregister on drop instead of panicking.
    pub fn silent_unregister_on_drop(mut self) -> Self {
        self.silent_drop = true;
        self
    }

    /// Build and register the buffer ring.
    pub fn build(self) -> io::Result<MmapedRingBuf> {
        let mut ring =
            MmapedRingBuf::new(self.buffer_count, self.buffer_size as u32, self.group_id)?;
        ring.silent_drop = self.silent_drop;
        ring.register()?;
        Ok(ring)
    }
}
...
/// # Cleanup
///
/// Call [`unregister()`](Self::unregister) before dropping, or the drop will panic.
/// Use [`MmapedRingBufBuilder::silent_unregister_on_drop()`] to disable the panic.
pub struct MmapedRingBuf {
...
impl MmapedRingBuf {
    fn new(buf_count: u32, buf_size: u32, group_id: u16) -> io::Result<Self> {
        if buf_count == 0 {
            return Err(io::Error::new(
        ...
        unsafe {
            shared_tail
                .as_ref()
                .store(buf_count as u16, Ordering::Release);
        }

        Ok(Self {
            data,
            buf_count: buf_count as u16,
    ...
    fn register(&mut self) -> io::Result<()> {
        ...
        let ptr = self.buf_ring_ptr.expect("MmapedRingBuf already unmapped");
    ...
    /// Unregister from io_uring, unmap memory, and consume the buffer ring.
    ...
    /// buf_id and len must come from a valid io_uring completion.
    #[inline]
    pub unsafe fn get_buf(&self, buf_id: u16, len: usize) -> RawBuffer<'_> {
    ...
    fn return_buffer(&self, buf_id: u16) {
        ...
        unsafe {
            ...
            let entry = &mut *ptr.as_ptr().add(ring_idx as usize);
...
impl Drop for MmapedRingBuf {
    fn drop(&mut self) {
        ...
        if self.silent_drop {
            ...
            panic!(
                "MmapedRingBuf dropped without calling unregister(). \
                You must call unregister() to properly clean up the buffer ring. \
                See MmapedRingBuf documentation for details."
            );
        }
    }
}

impl sealed::Sealed for MmapedRingBuf {}

impl RingBuf for MmapedRingBuf {
    ...
    unsafe fn get_buf(&self, buf_id: u16, len: usize) -> RawBuffer<'_> {
        MmapedRingBuf::get_buf(self, buf_id, len)
    }
}

/// Buffer borrowed from a [`MmapedRingBuf`].
pub struct RawBuffer<'ring> {
    buf_id: u16,
    offset: usize,
    len: usize,
    ring: &'ring MmapedRingBuf,
}
...
/// Buffer ring for recvmsg with address parsing. See [`MmapedRingBuf`] for cleanup.
pub struct MmapedRecvMsgRingBuf<P: RecvMsgParser> {
    inner: MmapedRingBuf,
    _parser: PhantomData<P>,
}

impl<P: RecvMsgParser> MmapedRecvMsgRingBuf<P> {
    /// Create new recvmsg buffer ring.
    pub fn new(buf_count: u32, payload_size: usize, group_id: u16) -> io::Result<Self> {
        let total_size = P::min_buffer_size() + payload_size;
        let mut inner = MmapedRingBuf::new(buf_count, total_size as u32, group_id)?;
        inner.register()?;
        Ok(Self {
            inner,
            _parser: PhantomData,
    ...
    #[inline]
    pub fn inner(&self) -> &MmapedRingBuf {
        &self.inner
    }
...
impl<P: RecvMsgParser> sealed::Sealed for MmapedRecvMsgRingBuf<P> {}

impl<P: RecvMsgParser> RingBuf for MmapedRecvMsgRingBuf<P> {
    type Buffer<'a>
        = RawBuffer<'a>
    where
        P: 'a;
...
impl<P: RecvMsgParser + Unpin + 'static> RecvMsgRingBuf for MmapedRecvMsgRingBuf<P> {
    type Parser = P;
